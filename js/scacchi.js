...
function buildPriceInfoText(t) {
  ...
  const courtMap = {
    compreso_gironi_finals: "Sede inclusa",
    compreso_gironi:        "Sede inclusa (solo gironi)",
    compreso_finals:        "Sede inclusa (solo finali)",
    non_compreso:           "Sede non inclusa",
    na:                     ""
  };
  const courtText = courtMap[courtPrice] ?? "Sede non inclusa";
...
function buildLocationInfoText(t) {
  ...
  const sedeLabel  = fixed === "fixed_all" ? "Sede prestabilita" : "Sede a scelta";
  return `${location} · ${sedeLabel}`;
}
...
function buildFormatInfoText(t) {
  ...
  if (guaranteed > 0) return `${formatText} · ${guaranteed} turni garantiti`;
