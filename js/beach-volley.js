...
const BEACH_VOLLEY_SPORTS = ["beach volley", "beach volleyball", "volley spiaggia", "pallavolo spiaggia"];

let ALL_TOURNAMENTS = [];

// ===============================
// HELPER: idempotente bool (frontend)
// Gestisce boolean nativi, stringhe "true"/"false"/"1"/"0", numeri
// ===============================
function toBool(val) {
...
function renderTournaments(tournaments) {
  ...
  tournaments.sort((a, b) => {
    // FIX BUG 5: fallback a 99 per status non mappati,
    // evita NaN nell'ordinamento per valori inattesi
  ...
  if (tournaments.length === 0) {
    container.innerHTML = "<p class='placeholder'>Nessun torneo di beach volley disponibile al momento.</p>";
    return;
  }
  ...
  tournaments.forEach(t => {
    ...
    // FIX BUG 2 (coerenza): toNum per teams_current e teams_max
...
function buildAwardInfoText(t) {
  // FIX BUG 1: toBool per gestire boolean nativo, stringa "true"/"false",
  // numero 1/0 — rimpiazza il doppio controllo manuale precedente
  ...
  // FIX BUG 3: toNum invece di Number() || 0 per gestire price/teams_max = 0
  const price    = toNum(t.price, 0);
  const teamsMax = toNum(t.teams_max, 0);
...
function buildFormatInfoText(t) {
  ...
  // FIX BUG 4: toNum invece di || 0 per gestire guaranteed_match
  // come stringa numerica (es. "3") o numero 0
